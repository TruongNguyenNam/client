...
// ✅ Export Excel thuộc tính sản phẩm
const exportAttributes = () => {
  if (!listProduct.value.length) {
    toast.add({ severity: 'warn', summary: 'Thông báo', detail: 'Không có dữ liệu để xuất', life: 3000 });
    return;
  }

  const data = listProduct.value.map((attr) => ({
    Name: attr.name,
    Description: attr.description,
  }));

  exportToExcel(data, 'DanhSachThuocTinh');
  toast.add({ severity: 'success', summary: 'Thành công', detail: 'Xuất Excel thành công', life: 3000 });
};

// ✅ Tải file Excel mẫu
const downloadAttributeTemplate = () => {
  downloadExcelTemplate(['Name', 'Description'], 'Template_ThuocTinh');
...
const importAttributes = async (event: any) => {
  ...
  try {
    ...
    const existingNames = listProduct.value.map(p => p.name.trim().toLowerCase());
    ...
    for (const item of importedData) {
      ...
      if (name) {
        ...
        if (!isDuplicate) {
          const attrData: ProductAttributeRequest = { name, description };
          await ProductAttributeService.saveProductAttribute(attrData);
          added++;
        } else {
    ...
    if (added > 0) {
      toast.add({
        ...
        detail: `Đã thêm ${added} thuộc tính từ file Excel.`,
        life: 4000,
      });
    }
    ...
    if (duplicatedNames.length > 0) {
      toast.add({
        severity: 'warn',
        summary: 'Bỏ qua trùng tên',
        detail: `Các thuộc tính bị trùng: ${duplicatedNames.join(', ')}`,
        life: 5000,
      });
    }
    ...
    toast.add({
      severity: 'error',
      summary: 'Lỗi',
      detail: 'Không thể import dữ liệu từ file Excel.',
      life: 4000,
    });
  }
};
...
<template>
  <div class="card">
    ...
    <DataTable :value="listProduct" :loading="loading" dataKey="id" :paginator="true" :rows="5"
      :rowsPerPageOptions="[5, 10, 20]" responsiveLayout="scroll" :first="first" @page="e => first = e.first"

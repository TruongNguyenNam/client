<template>
  ...
  import type { OrderResponse, CreateInvoiceRequest, OrderRequest, OrderItemRequest } from '../../../../model/order';
  ...
  const fetchAddresses = async () => {
    ...
    try {
      const customerId = selectedInvoice.value.userId;
      // Tìm thông tin khách hàng đã chọn
      const customer = customers.value.find(c => c.id === customerId);
      
      if (customer) {
        // Tạo một địa chỉ từ thông tin của khách hàng
        addresses.value = [{
          id: customer.id,
          streetAddress: customer.addressStreet,
          city: customer.addressCity,
          state: customer.addressState,
          country: customer.addressCountry,
          zipCode: customer.addressZipcode,
          ward: customer.addressWard,
  ...
  const completePayment = async () => {
    ...
    // Chuẩn bị payload gửi lên server - đã cập nhật với tên thuộc tính đúng
    ...
    // Thêm thông tin shipment nếu là đơn giao hàng
    if (!selectedInvoice.value.isPos && customer) {
      // Lấy danh sách id của các orderItem

...
<template>
  ...
  const fetchCart = async () => {
    ...
    try {
      ...
      cartItems.value = authStore.cart; // Sử dụng trực tiếp dữ liệu từ store
      console.log('Cart items updated:', cartItems.value.length); // Log để kiểm tra số lượng
      console.log('Cart count from store:', authStore.cartCount); // Log để kiểm tra cartCount
    } catch (error: any) {
